...
@MainActor
final class AppleMusicAppleScriptMonitor: ObservableObject {
    ...
    init() {
        // Music.app exposes the same player state vocabulary as Spotify but
        // duration and player position are in SECONDS (not ms like Spotify).
        let source = """
        ...
        tell application "Music"
            try
                ...
                return "music:track:" & theID & "|" & theName & "|" & theArtist & "|" & theAlbum & "|" & theDuration & "|" & thePosition & "|" & playerStateStr & "|" & ""
            on error errMsg
                return "ERROR|" & errMsg
            end try
        end tell
        ...
        guard let s = NSAppleScript(source: source) else {
            fatalError("Failed to compile Music AppleScript")
        }
    ...
    private func parse(_ raw: String) {
        ...
        let artist = fields[2]
        let album = fields[3]
        let durationSec = Double(fields[4]) ?? 0
